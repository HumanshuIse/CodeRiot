      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center flex-shrink-0">
            <button onClick={() => handleNavClick('/')} className="flex items-center space-x-2 text-white hover:text-cyan-400 transition-colors">
          ...
          <div className="hidden md:flex flex-grow items-center justify-center space-x-6">
            {navItems.map(({ id, label, icon: Icon, path }) => (
              <button key={id} onClick={() => handleNavClick(path)} className={`flex items-center space-x-1 px-4 py-2 rounded-xl transition-all duration-200 text-xs font-semibold uppercase ${ getActivePath(path) ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-300 hover:text-cyan-400 hover:bg-gray-800' }`}>
          ...
          <div className="hidden md:flex items-center flex-shrink-0">
            {isLoggedIn ? (
              <div className="flex items-center space-x-3">
                <button onClick={() => handleNavClick('/profile')} className="flex items-center space-x-2 text-gray-300 text-sm hover:text-cyan-400 transition-colors">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-gray-800 border-t border-gray-700">
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navItems.map(({ id, label, icon: Icon, path }) => (
              <button key={id} onClick={() => handleNavClick(path)} className={`flex items-center space-x-2 w-full px-3 py-2 rounded-lg transition-all duration-200 ${ getActivePath(path) ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-700' }`}>
                <Icon className="w-4 h-4" />
                <span>{label}</span>
              </button>
            ))}
            <div className="border-t border-gray-700 pt-2 mt-2">
              {isLoggedIn ? (
                <div className="space-y-1">
                  <button onClick={() => handleNavClick('/profile')} className="flex items-center space-x-2 w-full px-3 py-2 text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg transition-all duration-200">
                    <User className="w-4 h-4" />
                    <span>{username}</span>
                  </button>
